...
- 最新一档宿主版本改按其自身的发布时点解析依赖：隔离的 `0.1.5-rc.1` 安装现在传入 `--before`，因此在 registry 出现更新的 `0.1.5-rc.2` 兄弟组合后不再以 `ERESOLVE` 中止，而是完成自身依赖安装并跑出 157 项通过。
- 插件运行时无改动：宿主服务、客户端 bundle 与对外入口与 `0.1.18` 保持一致。
