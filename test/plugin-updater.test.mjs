...
test('归档会话独立更新只接受同源专用请求', () => {
  ...
  assert.equal(manualPluginUpdateCommand('web', '@ggtec528/dsh-archive-manager', '0.1.31'), 'dsh plugin --profile web add @ggtec528/dsh-archive-manager@0.1.31 --registry=https://registry.npmjs.org/')
...
test('归档客户端与 Host 绑定自身更新入口', async () => {
  ...
  assert.match(client, /packageName: "@ggtec528\/dsh-archive-manager"/)
  ...
  assert.match(host, /endpoint: "\/api\/ggtec528\/dsh-archive-manager\/update"/)
