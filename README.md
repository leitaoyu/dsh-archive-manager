...
| `0.1.5-rc.1` | `4.0.2` | 157 passed |

Counts above were measured on this fork under Linux / Node.js 24. The `0.1.5-rc.1` row is the newest supported version, and a newer sibling set (`0.1.5-rc.2`) exists in the registry, so a plain `npm install` resolves peers such as `dsh-session-title@^0.1.5-rc.1` to those newer siblings; their peer chain (`dsh-agent`, `dsh-system-prompt`, `dsh-tools`, `dsh-user-approval`, `dsh-invariants` at `^0.1.5-rc.2`) conflicts with the pinned `0.1.5-rc.1` set and npm aborts with `ERESOLVE` before any plugin code runs. This reproduces against untouched upstream `main`, whose dependency blocks are identical, and is a registry drift issue rather than a plugin regression. The isolated install for that row therefore passes `--before=2026-09-10T12:00:00.000Z`, the midpoint between the last `0.1.5-rc.1` publication and the first `0.1.5-rc.2` one, which resolves the whole `@deepseek-ai/dsh-*` tree to exactly `0.1.5-rc.1`; move that cutoff when the newest supported version changes.
