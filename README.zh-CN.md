...
| `0.1.5-rc.1` | `4.0.2` | 157 项通过 |

上表数量为本复刻分支在 Linux / Node.js 24 下实测。`0.1.5-rc.1` 是当前支持的最新版本，registry 中已存在更新的兄弟组合（`0.1.5-rc.2`）：直接 `npm install` 时 `dsh-session-title@^0.1.5-rc.1` 这类 peer 会被解析到更新的兄弟版本，而它们的 peer 链（`dsh-agent`、`dsh-system-prompt`、`dsh-tools`、`dsh-user-approval`、`dsh-invariants` 要求 `^0.1.5-rc.2`）与固定的 `0.1.5-rc.1` 组合冲突，npm 在运行任何插件代码前即以 `ERESOLVE` 中止。该现象在未改动的上游 `main` 上同样复现（两者依赖声明完全一致），属于 registry 漂移而非插件回归。因此该档的隔离安装会传入 `--before=2026-09-10T12:00:00.000Z`（最后一批 `0.1.5-rc.1` 发布与最早一批 `0.1.5-rc.2` 发布之间的中点），把整个 `@deepseek-ai/dsh-*` 依赖树解析为精确的 `0.1.5-rc.1`；最新支持版本变化时同步更新该分界点。
